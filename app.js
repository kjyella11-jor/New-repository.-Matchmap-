...
function initEventHandlers() {
  ...
  mobileToggle.addEventListener("click", () => {
    ...
    // Invalidate map size after mobile sidebar transitions to prevent broken/blank tiles
    setTimeout(() => {
      if (map) map.invalidateSize();
    }, 350); // Match CSS transition duration (0.3s) + slight buffer
  });
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // Watch window resize to invalidate map layout and prevent broken tiles
